defmodule ModernResumeWeb.Document.Renderer do
  @moduledoc """
  Document renderer module
   - Renders CV to LaTeX string or PDF
   - Uses Iona for LaTeX rendering and PDF generation
  """
  alias ModernResume.Resume.CV
  alias ModernResume.Resume.Experience
  alias ModernResume.Resume.Language
  alias ModernResume.Resume.Settings

  alias ModernResumeWeb.Document.Template

  @date_format "%b %Y"

  @escapes [
    {"{", "\\{"},
    {"}", "\\}"},
    {"\\", "\\textbackslash{}"},
    {"#", "\\#"},
    {"$", "\\$"},
    {"%", "\\%"},
    {"&", "\\&"},
    {"^", "\\textasciicircum{}"},
    {"_", "\\_"},
    {"|", "\\textbar{}"},
    {"~", "\\textasciitilde{}"}
  ]

  # Kanji Latex info
  # https://tex.stackexchange.com/questions/15516/how-to-write-japanese-with-latex

  @spec render(CV.t(), :str | :pdf) :: {:ok, String.t()} | {:error, String.t()}
  def render(%CV{} = cv, :str), do: {:ok, render_str(cv)}

  def render(%CV{} = cv, :pdf) do
    case render_pdf(cv) do
      {:ok, pdf} ->
        {:ok, Base.encode64(pdf)}

      _ ->
        {:error, "Error creating PDF"}
    end
  rescue
    _ ->
      {:error, "Error creating PDF"}
  end

  defp render_str(%CV{} = cv) do
    :moderncv
    |> Template.new()
    |> Template.eval(
      cv: cv.content,
      str: &str/1,
      date_range: &date_range/2,
      lang_fluency: &Language.display_fluency/1,
      employment_type: &Experience.display_employment_type/1,
      font_size: &Settings.display_font_size/1
    )
  end

  defp render_pdf(%CV{} = cv) do
    cv
    |> render_str()
    |> Iona.source()
    |> Iona.to(:pdf)
  end

  defp str(input) when is_binary(input) do
    Enum.reduce(@escapes, input, fn {pattern, replacement}, value ->
      String.replace(value, pattern, replacement)
    end)
  end

  defp str(value), do: value

  defp year_month_to_date(year, month) when is_integer(year) and is_integer(month) do
    case Date.new(year, month, 1) do
      {:ok, %Date{} = date} ->
        date

      {:error, _} ->
        nil
    end
  end

  defp year_month_to_date(_, _), do: nil

  defp date_range({start_year, start_month}, {end_year, end_month}) do
    date_start = year_month_to_date(start_year, start_month)
    date_end = year_month_to_date(end_year, end_month)

    get_date_range(date_start, date_end)
  end

  defp get_date_range(%Date{} = date_start, %Date{} = date_end) do
    with {:ok, str_start} <- Timex.format(date_start, @date_format, :strftime),
         {:ok, str_end} <- Timex.format(date_end, @date_format, :strftime) do
      "#{str_start} -- #{str_end}"
    else
      _ -> "n/a"
    end
  end

  defp get_date_range(%Date{} = date_start, _) do
    case Timex.format(date_start, @date_format, :strftime) do
      {:ok, str_start} -> "#{str_start} -- Present"
      _ -> "n/a"
    end
  end
end
